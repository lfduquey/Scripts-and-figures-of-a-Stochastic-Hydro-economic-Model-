...
Warning_Rules_LT<-function(uy,sdy,yo,uyhat,sdyhat,yohat,LT,n,IPrho,L,yT,
  ...
  clusterExport(cl,list("LT_PWS","uy","sdy","yo","uyhat","sdyhat","yohat","n","IPrho","L","yT","ybankfull","Affected_Houses","Vtheta","alpha","gama"))
  Results_LT<-parLapply(cl,LT, function (x) {
    LT_PWS(uy=uy,sdy=sdy,yo=yo,uyhat=uyhat,sdyhat=sdyhat,yohat=yohat,
           x,n=n,IPrho=IPrho,L=L,yT=yT,ybankfull=ybankfull,
  ...
  RPWSLT_DFDR<-data.frame(LT=unlist(LT,use.names = FALSE),
  ...
  RPWSLT_PDR<-data.frame(LT=unlist(LT,use.names = FALSE),
...
colnames(Skill_scores)<-c('LT','POD','FAR','Decision')
Skill_scores_LT<-melt(data =Skill_scores,
                      id.vars = c("LT", "Decision"),
...
Plot_Re_LT<-ggplot()+
  theme_classic()+
  geom_line(data = Skill_scores_LT,aes(x=LT,y=Value,color=Decision,linetype=Skill_Score),size=1.1)+
  ...
  labs(x= expression(tau), y="POD or FAR")


CT_baseline$LT<-as.character(CT_baseline$LT)
...
colnames(EW_CT_I)<-c('LT','Ew','Decision')
EW_CT_pf<-CT_baseline[(1:length(LT)),c(1,16)]
EW_CT_pf$Decision<-'pf'
colnames(EW_CT_pf)<-c('LT','Ew','Decision')
EW_CT<-rbind(EW_CT_I,EW_CT_pf)

EW_CT$LT<-as.numeric(EW_CT$LT)

Plot_Ew<-ggplot()+
  theme_classic()+
  geom_line(data = EW_CT,aes(x=LT,y=Ew,group=Decision,color=Decision),size=1.1)+
  scale_color_manual("Scenario",values = c("DFDR"="dodgerblue4","PDR"="gray","pf"="black"),
                     labels = c("DFDR"="DFS", "PDR"="PrFS", "pf"="PFS"))+
  labs(x=expression(tau),y=expression(E[w]~"[%]"))+
...
library(ggpubr)
png(filename="Figure5a1b1.png", 
    units="cm", 
    width=17, 
    height=8, 
    pointsize=8, 
    res=400)
ggpubr::ggarrange(Plot_Ew,Plot_Re_LT,ncol = 2,nrow = 1,labels = c('a','b'),
...
#-------------------------------------------- 
# Export results
save(Plot_Re_LT,CT_baseline,EW_CT,Plot_Ew,Skill_scores_LT,file = "Results_SA_LT.Rdata")
