...
nThouses<-1000
Vtheta<-1
alpha<-as.list(c(0.1,0.25,0.4,0.55,0.7,0.85,1))
gama<-0.1
################################################################################################################
################################################################################################################
# Calculations
...
Warning_Rules_LT<-function(uy,sdy,yo,uyhat,sdyhat,yohat,LT,n,IPrho,L,yT,ybankfull,Affected_Houses,Vtheta,alpha,gama){
  
  #---------------------------------------------------------------
  #---------------------------------------------------------------
  # Function to do the SA of the performance of the forecasting system
  ...
  LT_PWS<<-function(uy,sdy,yo,uyhat,sdyhat,yohat,LT,n,IPrho,L,yT,ybankfull,Affected_Houses,Vtheta,alpha,gama){
    ...
    ################################################################################################################
    # Probabilistic decision making for the property based criterion
    
    #---------------------------
    #---------------------------
    # Function to obtain the optima PT for the flooding-threshold based criterion
  ...
  #---------------------------------------------------------------
  #---------------------------------------------------------------
  # Run the scenario in parallel 
  
  library(parallel)
  no_cores <- detectCores() # Calculate the number of cores
  cl <- makeCluster(no_cores) # Initiate cluster
  clusterExport(cl,list("LT_PWS","uy","sdy","yo","uyhat","sdyhat","yohat","n",
                        "IPrho","L","yT","ybankfull","Affected_Houses","Vtheta","LT","gama"))
  Results_LT<-parLapply(cl,alpha, function (x) {
    LT_PWS(uy=uy,sdy=sdy,yo=yo,uyhat=uyhat,sdyhat=sdyhat,yohat=yohat,
           LT=LT,n=n,IPrho=IPrho,L=L,yT=yT,ybankfull=ybankfull,
           Affected_Houses=Affected_Houses,Vtheta=Vtheta,x,gama=gama)})
  stopCluster(cl)
  ...
  RPWSLT_DFDR<-data.frame(alpha=unlist(alpha,use.names = FALSE),
                          PT=rep(NA,length(unlist(LT,use.names = FALSE))),
                          h_houses=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$Hits_houses}),use.names = FALSE),
                          m_houses=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$Miss_houses}),use.names = FALSE),
                          f_houses=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$FA_houses}),use.names = FALSE),
                          cn_houses=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$CN_houses}),use.names = FALSE),
                          h1_houses=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$Hits1_houses}),use.names = FALSE),
                          h2_houses=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$Hits2_houses}),use.names = FALSE),
                          h1_fa_houses=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$Hits1_fa_houses}),use.names = FALSE),
                          h2_m_houses=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$Hits2_m_houses}),use.names = FALSE),
                          POD_houses=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$POD_houses}),use.names = FALSE),
                          FAR_houses=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$FAR_houses}),use.names = FALSE),
                          EAD_nw=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$EAD_nw}),use.names = FALSE),
                          EAD_PF=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$EAD_PF}),use.names = FALSE),
                          EAD_IF=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$EAD_DF}),use.names = FALSE),
                          Ew_PF=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$Ew_PF}),use.names = FALSE),
                          Ew_IF=unlist(lapply(Results_LT, function(x){x$Results_DDM$RDDM$Ew_DF}),use.names = FALSE),
                          Decision='DFDR',Type='FTC_FPC')
  
  
  RPWSLT_PDR<-data.frame(alpha=unlist(alpha,use.names = FALSE),
                         PT=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$PT}),use.names = FALSE),
                         h_houses=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$Hits_houses}),use.names = FALSE),
                         m_houses=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$Miss_houses}),use.names = FALSE),
                         f_houses=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$FA_houses}),use.names = FALSE),
                         cn_houses=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$CN_houses}),use.names = FALSE),
                         h1_houses=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$Hits1_houses}),use.names = FALSE),
                         h2_houses=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$Hits2_houses}),use.names = FALSE),
                         h1_fa_houses=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$Hits1_fa_houses}),use.names = FALSE),
                         h2_m_houses=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$Hits2_m_houses}),use.names = FALSE),
                         POD_houses=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$POD_houses}),use.names = FALSE),
                         FAR_houses=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$FAR_houses}),use.names = FALSE),
                         EAD_nw=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$EAD_nw}),use.names = FALSE),
                         EAD_PF=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$EAD_PF}),use.names = FALSE),
                         EAD_IF=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$EAD_PrF}),use.names = FALSE),
                         Ew_PF=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$Ew_PF}),use.names = FALSE),
                         Ew_IF=unlist(lapply(Results_LT, function(x){x$Results_PDR$Opt_PT$Ew_PrF}),use.names = FALSE),
                         Decision='PDR',Type='FTC_FPC')
  
  
  
  
  RLT<-rbind(RPWSLT_DFDR,RPWSLT_PDR)
  ...
  Results<-list()
  Results[['Cont_Table']]<-RLT
  Results[['Results_LT']]<-Results_LT
  return(Results)
}

# Run the analysis
Results_WDecisions<-Warning_Rules_LT(uy=uy,sdy=sdy,yo=yo,uyhat=uyhat,sdyhat=sdyhat,yohat=yohat,
                                     LT=LT,n=n,IPrho=IPrho,L=L,yT=yT,ybankfull=ybankfull,
...
################################################################################################################
# Export tables
CT_baseline<-Results_WDecisions$Cont_Table
write.csv(CT_baseline,'CT_baseline.csv')
...
Skill_scores<-CT_baseline[,c(1,11,12,18)]
colnames(Skill_scores)<-c('alpha','POD','FAR','Decision')
Skill_scores_alpha<-melt(data =Skill_scores,
                      id.vars = c("alpha", "Decision"),
                      variable.name = "Skill_Score",
                      value.name = "Value")

Skill_scores_alpha$alpha<-as.numeric(Skill_scores_alpha$alpha)

Plot_Re_alpha<-ggplot()+
  theme_classic()+
  geom_line(data = Skill_scores_alpha,aes(x=alpha,y=Value,color=Decision,linetype=Skill_Score),size=1.1)+
  scale_color_manual("Scenario",values = c("DFDR"="dodgerblue4","PDR"="gray"),
                     labels=c("DFDR"="DFS", "PDR"="PrFs"))+
  scale_linetype_manual('',values = c("POD"="solid","FAR"="dotted"))+
  labs(x= expression(alpha), y="POD or FAR")

CT_baseline$alpha<-as.character(CT_baseline$alpha)

#--------------------------------------------
#-------------------------------------------- 
# Effectivness

EW_CT_I<-CT_baseline[,c(1,17,18)]
colnames(EW_CT_I)<-c('alpha','Ew','Decision')
EW_CT_pf<-CT_baseline[(1:length(alpha)),c(1,16)]
EW_CT_pf$Decision<-'pf'
colnames(EW_CT_pf)<-c('alpha','Ew','Decision')
EW_CT<-rbind(EW_CT_I,EW_CT_pf)

EW_CT$alpha<-as.numeric(EW_CT$alpha)

Plot_Ew<-ggplot()+
  theme_classic()+
  geom_line(data = EW_CT,aes(x=alpha,y=Ew,group=Decision,color=Decision),size=1.1)+
  scale_color_manual("Scenario",values = c("DFDR"="dodgerblue4","PDR"="gray","pf"="black"),
                     labels = c("DFDR"="DFS", "PDR"="PrFS", "pf"="PFS"))+
  labs(x=expression(alpha),y=expression(E[w]~"[%]"))+
...
library(ggpubr)
png(filename="Figure5a2b2.png", 
    units="cm", 
    width=18, 
    height=9, 
    pointsize=8, 
    res=400)
ggpubr::ggarrange(Plot_Ew,Plot_Re_alpha,ncol = 2,nrow = 1,labels = c("a","b"),
...
save(Plot_Re_alpha,Skill_scores_alpha,CT_baseline,EW_CT,Plot_Ew,file = "Results_SA_Alpha.Rdata")
